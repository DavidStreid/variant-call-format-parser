...
exports.extractVariantInfo = function(vcfFile) {
  const cleanVcf = cleanVcf(vcfFile);

  const data = getVariantLines(cleanVcf);
  const columns = getColumns(cleanVcf);
  const mutationInfo = [];
...
exports.extractMetadata = function(vcfFile) {
  const cleanVcf = cleanVcf(vcfFile);
  const data = getMetadatLines(cleanVcf);
